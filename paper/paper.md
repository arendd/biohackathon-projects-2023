...
### e!DAL-PGP
e!DAL-PGP (Plant Genomics and Phenomics Research Data Repository) [@citesAsAuthority:Arend_2016] is an infrastructure to comprehensively publish plant research data. This covers in particular cross-domain datasets that are not being published in central repositories because of its volume or unsupported data scope, like image collections from plant phenotyping and microscopy, unfinished genomes, genotyping data, visualizations of morphological plant models, data from mass spectrometry as well as software and documents. All published datasets are accessible via a DOI linking to a dedicated content page, which embeds a Schema.org and Bioschemas markup in the HTML code. Currently, it supports the `Dataset` and `Taxon` profiles.
